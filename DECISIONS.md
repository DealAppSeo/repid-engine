...
## 9. zkRepID Is the Canonical Name — For a Narrower Boundary Than `src/zkp/` (2026-08-17)
- **Decision**: `zkRepID` is the canonical name for the layer that proves a RepID without revealing it. It names the **six RepID-specific modules**, not the whole of `src/zkp/`. Decided by Sean, 2026-08-17, answering the open question in `docs/RSI-ADOPTION-PLAN.md` §8. Boundary: `src/zkrepid/boundary.ts`; prose: `docs/ZKREPID.md`.
- **Context**: `zkRepID` previously named no code in any of the four repos, which is the failure LESSONS §5 describes. The obvious way to fix that was `git mv src/zkp src/zkrepid`. Scoping showed it would be wrong on the merits: `src/zkp/` holds general-purpose zero-knowledge machinery — Poseidon2 (a hash function), Plonky3 (a proving backend), a self-described hash-agnostic Merkle implementation, and the separate `zkp-vault` Rust crate that owns a required CI check — alongside six modules that exist only because a *RepID* is the thing being proved. Attaching a product name to general cryptographic primitives would leave the codebase less able to say which is which. Two constraints made a wholesale rename impossible in any case: the `zkp_circuit` / `zkp_proof_cid` / `zkp_sbt_proof_cid` / `custodian_zkp_proof` columns live in an externally-managed Supabase schema this repo does not own (no migrations live here), and `zkp-vault` is a Rust crate. A full rename would therefore have landed half-applied, and a name that is canonical in some places and not others is not a canonical name.
- **Outcome**: The name is made real the way a name should be — by saying precisely what it covers, backing it with importable code, and pinning it with tests. `src/zkrepid/index.ts` is a **purely additive** namespaced re-export barrel: no file moved, no import changed, no behaviour altered. Namespaced rather than flat because `feltsFromString` is exported by both `repid-delta-statement` and `nullifier-identity` under different domain-separation tags, and a flat `export *` would silently resolve one — a digest computed under the wrong intent. The barrel excludes `repid-delta-bridge` (which is zkRepID but imports `../db` and throws at module scope without credentials), so the canonical entry point imports with no environment set and is never harder to use than the modules beneath it. `tests/zkrepid-boundary.test.ts` enforces the boundary, requires every module under `src/zkp/` to be classified as zkRepID or explicitly not, and checks `docs/ZKREPID.md` against the code in both directions so the definition cannot drift from what it defines.
