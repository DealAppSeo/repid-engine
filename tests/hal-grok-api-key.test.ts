 ...
 * precision lever dead. `grokApiKey()` now resolves `XAI_API_KEY || GROK_API_KEY`. These tests lock
 * that both names work and that the canonical name wins, so the lever can never silently die again.
 *
 ...
 * on both surfaces now, and `tests/grok-key-precedence-parity.test.ts` keeps them that way.
 */
