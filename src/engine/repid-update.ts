...
export interface RepIdUpdateInput {
  ...
  eventType:
    ...
    | 'PEER_VERIFY_WRONG_CALL'; // Phase 3 dogfooding (behind DOGFOOD_REPID_FROM_COSIGN)
...
const FIXED_DELTAS: Partial<Record<RepIdUpdateInput['eventType'], number>> = {
  ...
  PEER_VERIFY_WRONG_CALL: -5, // BFT panel: panelist diverged from majority (bounded; low-confidence self-flag exempt at the emitter)
};

/**
 * Co-sign / panel events that only move RepID when DOGFOOD_REPID_FROM_COSIGN=true.
 * PARITY: all three (verified award, false-pass slash, panel divergence slash) are gated together — so a
 * PEER_VERIFY_WRONG_CALL -5 can never apply live while the +10 award stays gated OFF (the asymmetric-dogfood
 * hole XC flagged). Exported so the gate is unit-testable without the DB.
 */
export function isCosignGatedEvent(t: RepIdUpdateInput['eventType']): boolean {
  return t === 'HANDOFF_COSIGN_VERIFIED' || t === 'HANDOFF_COSIGN_FALSE_PASS_SLASH' || t === 'PEER_VERIFY_WRONG_CALL';
}
...
export async function updateRepId(input: RepIdUpdateInput): Promise<RepIdUpdateResult> {
  ...
  const isDogfoodCoSignEvent = isCosignGatedEvent(input.eventType);
