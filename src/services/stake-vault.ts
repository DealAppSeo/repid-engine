...
/**
 * Integer Babylonian (Newton's method) sqrt. Loops until convergence —
 * the sprint spec's 8-iteration cap is insufficient for the magnitudes
 * we deal with (USDC smallest units routinely exceed 1e9).
 *
 * Returns floor(sqrt(n)). Guaranteed terminating: each iteration
 * monotone non-increasing, returns when it stops improving.
 */
export function babylonianSqrt(n: bigint): bigint {
  if (n < 0n) return 0n;
  if (n === 0n) return 0n;
  if (n < 4n) return 1n;
  // Start ABOVE sqrt(n) so Newton's method monotonically descends. Using
  // (bits+1)/2 guarantees y >= sqrt(n) at iteration 0; the iteration then
