 ...
 *
 * An earlier revision of this comment named a money-path gate environment
 * variable that did not exist [MEASURED 2026-09-08]: the name appeared nowhere
 * in this repository except that sentence — not in code, not in the generated
 * env registry. A named env var reads as a real control, so the sentence
 * invented a safety mechanism a reader would then assume was protecting the
 * money path. The real money-path gate is `X402_ENFORCEMENT_ENABLED`
 * (`src/index.ts`, `processCascadeQueue`), and it gates escrow admission, not
 * the delta.
 *
