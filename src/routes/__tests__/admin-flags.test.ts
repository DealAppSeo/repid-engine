...
describe('Admin Flags', () => {
  ...
  it('EAS_ANCHOR_WORKER_ENABLED defaults false, with a note naming the worker and the attester-key precondition', async () => {
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
    expect(res.body.eas_anchor_worker_enabled).toEqual({
      value: false,
      source: 'default',
      note: expect.stringContaining('easAnchorWorker'),
    });
    expect(res.body.eas_anchor_worker_enabled.note).toEqual(expect.stringContaining('attester key'));
  });

  it('EAS_ANCHOR_WORKER_ENABLED=true -> reports true, source env', async () => {
    process.env.EAS_ANCHOR_WORKER_ENABLED = 'true';
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
  ...
  it('X402_ENFORCEMENT_ENABLED defaults false, with a note naming the legacy no-payment-check branch', async () => {
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
    expect(res.body.x402_enforcement_enabled).toEqual({
      value: false,
      source: 'default',
      note: expect.stringContaining('/escrow'),
    });
    expect(res.body.x402_enforcement_enabled.note).toEqual(expect.stringContaining('no X-PAYMENT header'));
  });

  it('X402_ENFORCEMENT_ENABLED=true -> reports true, source env', async () => {
  ...
  it('HUMAN_AGENT_BIND_ENABLED defaults false, with a note naming human-agent-binding.ts', async () => {
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
    expect(res.body.human_agent_bind_enabled).toEqual({
      value: false,
      source: 'default',
      note: expect.stringContaining('human-agent-binding.ts'),
    });
  });
  ...
  it('AGENT_SELF_SERVE_KEYS_ENABLED defaults false, with a note naming agent-self-serve-key.ts', async () => {
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
    expect(res.body.agent_self_serve_keys_enabled).toEqual({
      value: false,
      source: 'default',
      note: expect.stringContaining('agent-self-serve-key.ts'),
    });
  });

  it('AGENT_SELF_SERVE_KEYS_ENABLED=true -> reports true, source env', async () => {
  ...
  it('EXECUTION_FLOOR_ENABLED defaults false, with a note naming execution-floor.ts', async () => {
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
    expect(res.body.execution_floor_enabled).toEqual({
      value: false,
      source: 'default',
      note: expect.stringContaining('execution-floor.ts'),
    });
  });

  it('EXECUTION_FLOOR_ENABLED=true -> reports true, source env', async () => {
    process.env.EXECUTION_FLOOR_ENABLED = 'true';
    const res = await request(app).get('/api/v1/admin/flags').set('x-admin-key', 'secret');
