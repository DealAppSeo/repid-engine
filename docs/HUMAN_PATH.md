...
| 3 | Testnet tokens | `GET /api/v1/faucet/info`, `GET /api/v1/faucet/balance` | Read-only. `dispenses: false`. Names the public Base Sepolia faucets. The chain id is whatever `getActiveNetwork()` reports. | Records chain id **84532** and `dispenses: false`. Does not dial an RPC. |
...
| 5 | Bind agents | `POST /api/v1/human/bind` | Bind is a signature that names the wallet and the agent. `HUMAN_AGENT_BIND_ENABLED` defaults off. One live owner per agent per scope. | Publishes that flag the same way as connect. Inserts no binding. |
