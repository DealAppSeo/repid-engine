...
This divergence is not academic. The cross-crate test
(`tests/zkp-proof-verifier-crosscheck.test.ts`) verifies a **range-check** proof through
`@hyperdag/proof-verifier` — that circuit is built at the **aggregation** git-rev pin
(`27d59f7350`), and its proof cannot be produced by `zkp-vault`, whose leaf circuit
proves a different statement (anonymous membership/nullifier) at the crates.io `0.3.0`
pin. The two tiers are genuinely separate proof systems today; the synthetic fixture for
that test is therefore generated with the aggregation-tier circuit's own prover, not with
`zkp-vault`. See `scripts/zkp/gen-synthetic-rangecheck-proof.rs`.
...
survives. Per ZKP Invariant 5, Sean's standing call is STAY NATIVE (recursion optional),
so this reconciliation is a reviewed decision, not an automatic one.
