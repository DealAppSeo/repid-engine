...
const sqls = [
  ...
  `CREATE TABLE IF NOT EXISTS repid_stakes (
    id uuid primary key default gen_random_uuid(),
  ...
  `CREATE TABLE IF NOT EXISTS repid_trade_attempts (
    id uuid primary key default gen_random_uuid(),
    agent_id uuid not null references repid_agents(id),
    user_id uuid not null references repid_users(id),
  ...
  ON CONFLICT (agent_id) DO UPDATE SET repid_score = EXCLUDED.repid_score`
];
