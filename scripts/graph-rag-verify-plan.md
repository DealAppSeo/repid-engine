...
```powershell
# On Windows PowerShell, prefer direct invocation — npm strips the `--`
# separator on Windows so the `--dry-run` flag won't propagate through
# `npm run`. Either of these works:
npx ts-node scripts/graph-rag-verify.ts --dry-run
# or just (without --dry-run, the harness still exits cleanly on pre-migration)
npm run verify:graph-rag
```

**Expected (pre-migration):** A1, A2, A3 all ❌ + `⏸  Migration not yet applied.` + exit 0. This is the documented pre-migration state. Confirmed live by CC1 on 2026-05-10 evening against `qnnpjhlxljtqyigedwkb`.
